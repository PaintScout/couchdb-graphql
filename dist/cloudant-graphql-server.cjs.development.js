...
function createServer(_temp) {
  ...
  var base = apolloServer.gql(_templateObject$9());
  var server = new apolloServer.ApolloServer({
    schema: federation.buildFederatedSchema([{
      typeDefs: base
    }].concat(Object.keys(queries).map(function (key) {
      return queries[key];
    ...
    context: function (args) {
      try {
        var _temp4 = function _temp4() {
          ...
          if (!dbName) {
            throw new apolloServerCore.AuthenticationError('Authentication required');
          }
